...
namespace SAP
{
    public class SAPInterface
    {
        ...
        private EntityShipMethod GetShipViaCode(DataRow dr)
        {
            ...
            if (dr["ThirdPartyAccount"] != null) objSM.strPayorAccountNumber = dr["ThirdPartyAccount"].ToString().Trim();
            if (objSM.strPayorAccountNumber.Length > 1) strPaymentType = "3RD";
        ...
        private EntityAddress FillBillTo(DataRow dr)
        {
            ...
            try
            {
                if (dr["ThirdPartyAccount"] != null) objBT.strAccountNumber = dr["ThirdPartyAccount"].ToString().Trim();
                if (dr["BillToZip"] != DBNull.Value)
                {
                    objBT.strPostalCode = dr["BillToZip"].ToString().Trim();
                    if (dr["BillToCompany"] != DBNull.Value) objBT.strCompanyName = dr["BillToCompany"].ToString().Trim();
                    if (dr["BillToAddress1"] != DBNull.Value) objBT.strAddressLine1 = dr["BillToAddress1"].ToString().Trim();
                    if (dr["BillToAddress2"] != DBNull.Value) objBT.strAddressLine2 = dr["BillToAddress2"].ToString().Trim();
                    if (dr["BillToCity"] != DBNull.Value) objBT.strCity = dr["BillToCity"].ToString().Trim();
                    if (dr["BillToState"] != DBNull.Value) objBT.strState = dr["BillToState"].ToString().Trim();
                    if (dr["BillToAttention"] != DBNull.Value) objBT.strContactName = dr["BillToAttention"].ToString().Trim();
...
//            Int32.TryParse(objES.objDetails.lstReference[3].strReferenceValue, out intPlant);
            ...
            do
            {
                SqlCommand sqlCMD = objSQLCon.CreateCommand();
                {
                    ...
                    sqlCMD.CommandText = "INSERT INTO dbo.Shipments (Reference1, Reference2, Reference3, Reference4, Reference5, VoidIndicator, CompanyName, AttentionTo, Address1, Address2, Address3, City, ShipState, ZipCode, Country, ShippedBy, TrackingNumber, ShipWeight, TotalContainers, ThisContainer, CarrierCharge, ListCharge, SourceSystem, SQLStatus, ShipDate, LastModified, BOL, CarrierNumber, PlantNumber, PhoneNumber, ServiceType, ServiceName, SenderAccount, AdditionalCharge, ProcessDate, TerminalID, ProcessStatus, ThirdPartyAccount, CustomerReference, SenderEmailAddress1, SenderEmailNotify1, SenderEmailAddress2, SenderEmailNotify2, SenderEmailAddress3, SenderEmailNotify3, SenderEmailAddress4, SenderEmailNotify4, ResidentialStatus, EmailText, SignatureOption, SaturdayDelivery, BillToCompany, BillToAttention, BillToAddress1, BillToAddress2, BillToCity, BillToState, BillToZip, ReturnAddressCompany, ReturnAddressAttention, ReturnAddressLine1, ReturnAddressLine2, ReturnAddressCity, ReturnAddressState, ReturnAddressZip, Instructions) VALUES (@Reference1, @Reference2, @Reference3, @Reference4, @Reference5, @VoidIndicator, @CompanyName, @AttentionTo, @Address1, @Address2, @Address3, @City, @ShipState, @ZipCode, @Country, @ShippedBy, @TrackingNumber, @ShipWeight, @TotalContainers, @ThisContainer, @CarrierCharge, @ListCharge, @SourceSystem, @SQLStatus, @ShipDate, @LastModified, @BOL, @CarrierNumber, @PlantNumber, @PhoneNumber, @ServiceType, @ServiceName, @SenderAccount, @AdditionalCharge, @ProcessDate, @TerminalID, @ProcessStatus, @ThirdPartyAccount, @CustomerReference, @SenderEmailAddress1, @SenderEmailNotify1, @SenderEmailAddress2, @SenderEmailNotify2, @SenderEmailAddress3, @SenderEmailNotify3, @SenderEmailAddress4, @SenderEmailNotify4, @ResidentialStatus, @EmailText, @SignatureOption, @SaturdayDelivery, @BillToCompany, @BillToAttention, @BillToAddress1, @BillToAddress2, @BillToCity, @BillToState, @BillToZip, @ReturnAddressCompany, @ReturnAddressAttention, @ReturnAddressLine1, @ReturnAddressLine2, @ReturnAddressCity, @ReturnAddressState, @ReturnAddressZip, @Instructions)";
